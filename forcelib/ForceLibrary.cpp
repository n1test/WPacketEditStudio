					...
					DWORD dwCodeStart)
{
	...
	if (LibLoaderCode)
	{
		...
		strcpy_s(LibLoaderCode->LibPath, 260, szTargetLib);
	...
	else
	{
		...
		strcpy_s(LibLoaderCodeNT->LibPath, 260, szTargetLib);
...
BOOL InitCodeStructDBG(sLibLoadCodeDBG &LibLoaderCode,CHAR* szTargetLib,DWORD dwCodeStart)
{
	...
	strcpy_s(LibLoaderCode.LibPath, 256, szTargetLib);
...
DWORD GetProcessEntryPoint(DWORD PID)
{
	...
	while (_Process32Next(hSnap,&ProcInfo))
		if (ProcInfo.th32ProcessID == PID)
			strcpy_s((LPTSTR)&ProcPath, 260, ProcInfo.szExeFile);
	CloseHandle(hSnap);
	if (ProcPath[0] == 0)
		return 0;
	...
	if (_stricmp((LPCTSTR)&ModuleInfo.szExePath,(LPCTSTR)&ProcPath) == 0)
	{
