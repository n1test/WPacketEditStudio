...
void C3DMeterCtrl::DrawMeterBackground(CDC *pDC, CRect &rect)
{
  ...
  m_nValueFontHeight = tm.tmHeight ;
	...
	strTemp.Format("%.*lf%s", m_nScaleDecimals, m_dMinValue, (LPCSTR)m_Unit) ;
	...
	strTemp.Format("%.*lf%s", m_nScaleDecimals, m_dMaxValue, (LPCSTR)m_Unit) ;
...
void C3DMeterCtrl::SetRange(double dMin, double dMax, bool update = true) 
{
	m_dMaxValue = dMax ;
	m_dMinValue = dMin ;
	if(update)
...
void C3DMeterCtrl::SetTitle(LPSTR lpTitle)
{
	m_strUnits.Format(lpTitle);
}

void C3DMeterCtrl::SetUnit(LPSTR lpUnit)
{
	m_Unit = (CString)lpUnit;
}
